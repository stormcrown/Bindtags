...
class DeleteRackInfoTask(currentActivity: Activity, webserviceUrl: String) : BaseTask(currentActivity, webserviceUrl) {
    ...
    override fun onPostExecute(result: String) {
        try {
            Log.i("DeleteRackInfoTask",result)
...
//                msg = flag.getString("msg")
                isSuccess= flag.getString("isSucceed")
            }
            val bundle = Bundle()
            val message =Message()
            bundle.putString("isSuccess",isSuccess)
//            if(!TextUtils.isEmpty(msg))bundle.putString("msg",msg)
            message.what = RackDetailActivity.handlerDeleteResult
            message.data = bundle
